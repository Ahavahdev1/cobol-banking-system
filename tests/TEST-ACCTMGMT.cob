...
TEST-AM-028.
    ADD 1 TO WS-TEST-COUNT
    MOVE "AM-028: CLOS past-due!=0 -> E0048" TO WS-TEST-NAME
    INITIALIZE ACCT-RECORD
    INITIALIZE WS-ACCT-RESULT
    PERFORM SETUP-VALID-CIF
    MOVE "CLOS" TO WS-FUNCTION
    ...
    CALL "ACCTMGMT" USING WS-FUNCTION ACCT-RECORD
                          CIF-RECORD WS-ACCT-RESULT
    IF WS-ACCT-RESULT-CODE = "E0048"
        ADD 1 TO WS-PASS-COUNT
        DISPLAY "  PASS: " WS-TEST-NAME
    ELSE
        ADD 1 TO WS-FAIL-COUNT
        DISPLAY "  FAIL: " WS-TEST-NAME
            " expected=E0048 actual=" WS-ACCT-RESULT-CODE
    END-IF.

TEST-AM-029.
    ADD 1 TO WS-TEST-COUNT
    MOVE "AM-029: CLOS escrow!=0 -> E0049" TO WS-TEST-NAME
    INITIALIZE ACCT-RECORD
    INITIALIZE WS-ACCT-RESULT
    PERFORM SETUP-VALID-CIF
    MOVE "CLOS" TO WS-FUNCTION
    ...
    MOVE 1200.00 TO ACCT-ESCROW-BAL
    CALL "ACCTMGMT" USING WS-FUNCTION ACCT-RECORD
                          CIF-RECORD WS-ACCT-RESULT
    IF WS-ACCT-RESULT-CODE = "E0049"
        ADD 1 TO WS-PASS-COUNT
        DISPLAY "  PASS: " WS-TEST-NAME
    ELSE
        ADD 1 TO WS-FAIL-COUNT
